...
### API Documentation
* **[POST]** * to s`http://localhost:9000/api/login`: returns a the current authenication information of the user. Pass in the following credential as the `body` of the request: `{ username: 'Lambda', password: 'School' }` for a successful login.
...
1. Explain what a token is used for.
    - An object that can be used to authenticate a user to a server.
2. What steps can you take in your web apps to keep your data secure?
    - encrypting passwords
    - USe a secure host
3. Describe how web servers work.
    - web servers work by receiving a request from a client then sending back a response back to the client.
